...
- [ ] User-settable inflow marker: let an inlet/source inject already-burnt gas (`marker=1`, e.g. exhaust-gas recirculation as a feed) instead of the hardcoded fresh `0`. The plumbing carries it (the marker is just another boundary scalar param); only the catalog factory + YAML key are missing.
