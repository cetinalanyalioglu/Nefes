...
- [ ] Heat-release n-tau flame source S(omega) in `stamp_sources`: lights up active/thermoacoustic (growing) modes in `eigenmodes()` (the Beyn driver is already source-agnostic). Needs a steady heat-release element kernel + complex-step probe; validate with a Rijke tube.
