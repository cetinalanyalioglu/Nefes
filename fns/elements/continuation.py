...
contour eigensolver.  Because it is a :class:`~fns.elements.dynamic_source.TransferFunction`
...
class RationalFit(TransferFunction):
    ...
    algorithm and exposes the result as a :class:`~fns.elements.dynamic_source.TransferFunction`
    ...
    def poles_in_region(self, freq_band, growth_band=None) -> np.ndarray:
        ...
        the :func:`~fns.perturbation.eigenmodes` convention.
    ...
    def plot_fit(self, **kwargs):
        """Overlay the continued curve on the tabulated data (see :func:`fns.plotting.plot_fit`)."""
        from ..plotting import plot_fit

        return plot_fit(self, **kwargs)

    def plot_pole_map(self, **kwargs):
        """Pole/zero map in the frequency-growth plane (see :func:`fns.plotting.plot_pole_map`)."""
...
def rational_fit(freqs, values, **kwargs) -> RationalFit:
    ...
    complex frequency), where a real-grid :func:`~fns.elements.dynamic_source.tabulated`
