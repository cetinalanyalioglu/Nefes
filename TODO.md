...
- [ ] Balance/scale the perturbation operator `A(omega)` before *factorization* (the equilibration in `eigenmodes._ResidualScale` currently only rescales the residual measure), or nondimensionalize omega by a reference `c/L`, so the dominant boundary/regularizer entries no longer swamp the rest and the sparse solves themselves are better conditioned. Note: a similarity transform preserves eigenvalues, so it improves conditioning but does not thin the dense convected spectrum (that stays the Nyquist regime).
...
- [ ] Composite-element follow-ups (the v1 build ships Part-I/II append-at-tail): (a) bandwidth-aware renumber of expanded internals into the Solve space (gated on the re-order algorithm above; `splu` re-permutes so append is harmless today -- `scratch/composite-elements.md` Part III); (b) Class-3 sub-network composites (branching internals via the indexed port-wiring table -- `transfer_matrix` across a branch point already warns, use `multiport_scattering_matrix`).
...
- [ ] Parameter API step 4 (canonical-store flip): make the named-parameter dict the stored truth on every spec and `fparams` a derived, invalidatable cache. Guard with a per-kind projection round-trip invariant plus the complex-step safety sweep; pays down the composite/taper IO round-trip debt and is prerequisite work for subnetworks. See `docs/design/parameters.md`.
- [ ] Parameter API step 6 (subnetworks): recursive node type (named dict + children) with hierarchical addressing (`can[3].injector.orifice.throat_area`) and template/instance overlays; settle the repeated-instance address syntax (`can[3]` vs `can.3`) before landing. Requires the store flip above.
