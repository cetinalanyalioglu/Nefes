...
from nefes.assembly.recover import ES_M, ES_PT, ES_RHO, ES_U
...
def _contraction(cc, A_large=0.09, A_small=0.05, pt_in=120000.0, p_out=101325.0):
    ...
    prob = build_problem(CFG, net, [(0, 1, A_large), (1, 2, A_small)], 10.0, 101325.0, CP * 300.0)
    res = solve(prob)
...
def test_expansion_unaffected_by_cc():
    ...
    def expand(cc):
        ...
        prob = build_problem(CFG, net, [(0, 1, 0.05), (1, 2, 0.09)], 10.0, 101325.0, CP * 300.0)
        res = solve(prob)
        assert res.converged
