...
## Edge-state recovery precedes the residuals {#sec-assembly-recovery}
...
## The fixed row split {#sec-assembly-split}

The assembled system is partitioned into two blocks whose sizes do not depend on the flow direction, which is the structural realization of *discovery over prescription* (see [equation structure](../theory/equation-structure.md#sec-eqstruct-fixed-split)).
...
## The sparse Jacobian by complex step {#sec-assembly-jacobian}
...
## Acoustic stamps as a low-rank layer {#sec-assembly-stamps}
...
The globalization that turns Newton's local method into a solver robust from a cold start is [the solver](solver.md).
