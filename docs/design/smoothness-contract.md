...
4. **Marker gate** — a re-centred, normalized smoothed step with $g(0) = 0$ and $g(1) = 1$ exact, gating the frozen and burnt reacting closures (see [thermochemistry](../theory/thermochemistry.md)).
...
6. **Smoothed Fischer–Burmeister complementarity**, $a + b - \sqrt{a^2 + b^2 + \varepsilon^2}$ — encodes an either/or regime switch as a single residual; the choking complementarity (see [choking](../theory/choking.qmd)).
