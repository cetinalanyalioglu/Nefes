...
Its aggressiveness is adjustable through three settings on `equilibrium()`: the reducer itself (`reducer`, with `"none"` keeping every candidate), the trace mole-fraction cutoff below which a species is dropped (`reduce_threshold`), and the candidate count above which the reduction runs at all (`reduce_above`).
