...
It is worth recording that this failure is not peculiar to compressible networks; it is the same degeneracy that hydraulic pipe-network solvers have long contended with, reached by a different physical route.
There the pressure–flow coupling comes from the friction loss law $\Delta p = R\,|Q|^{n-1}Q$ with $n$ between $1.85$ and $2$ rather than from the dynamic head, but its derivative $nR|Q|^{n-1}$ vanishes at rest just the same.
@elhay_simpson_2011 prove the consequence for the global gradient algorithm of @todini_pilati_1988: the Jacobian is singular whenever the zero-flow pipes form a closed loop or a path between two fixed-head nodes, and the algorithm fails outright by division by zero.
Their remedy — replacing the loss law of a nearly stagnant pipe by a linear surrogate — is the same first-order sensitivity injected below, differing only in that it is applied locally and kept permanently, which perturbs the converged answer [@gorev_2022], whereas the continuation used here removes it (see [solver](../design/solver.md#sec-solver-continuation)).
