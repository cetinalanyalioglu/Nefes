...
## The de-embedding principle {#sec-ident-de-embedding}
...
## Recovering a two-port transfer matrix {#sec-ident-two-port}
...
## Recovering a flame transfer function {#sec-ident-flame-ftf}
...
## Isentropic versus full identification {#sec-ident-isentropic-full}
...
## Identifiability and its diagnostic {#sec-ident-identifiability}
...
## From data to the complex plane {#sec-ident-complex-plane}

The identification returns a real-frequency table of the recovered response and, by default, its analytic continuation.
The raw table lives only on the real axis, which suffices for a forced response and for the real-axis Nyquist stability driver of [analyses](analyses.qmd#sec-analyses-open-loop-stability), both of which evaluate only there.
