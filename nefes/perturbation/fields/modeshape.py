...
def resolve_specs(variable=None, basis=None):
    ...
    * ``basis`` -- a flavor from :data:`nefes.perturbation.operator.characteristics.BASIS_LABELS`
...
class NetworkGeometry:
    ...
    Attached to :class:`~nefes.perturbation.EigenmodeResult` and
    :class:`~nefes.perturbation.PerturbationResponse` so they can reconstruct the
...
def _adjacency(geo: NetworkGeometry):
    """``node -> list of (edge, neighbour node)`` (undirected over the Nefes edges)."""
