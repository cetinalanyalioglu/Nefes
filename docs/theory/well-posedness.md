...
The presentation begins by making that criterion precise, then proceeds through the three root causes in turn — the degeneracy of flux-form energy balances, the discontinuity of hard switches, and the first-order flatness of pressure-driven networks at rest — and closes with a milder, genuinely physical indeterminacy that the solver's damping absorbs.

## What ill-posedness means here
...
## Root cause B: hard switches destroy smoothness and rank
...
At a deeper structural level, the classical practice of *reassigning* which equation a port owns when the flow reverses changes the equation count on each side of the reversal, a discontinuity in the very shape of the system (see [equation structure](equation-structure.md)).
Both disappear under the constructions adopted earlier: the fixed per-edge split of [equation structure](equation-structure.md) keeps the equation count invariant to flow direction, and the smooth upwind weight $\theta = \operatorname{sstep}(\dot m;\varepsilon)$ of [transport](transport.qmd) selects the upstream side continuously, so the residual and its Jacobian remain smooth through reversal.

## Root cause C: zero flow is a stationary point of pressure-driven networks
